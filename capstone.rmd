...
## Normal connections
```{r}
normal_connections <- data[data$class == "normal", ]
...
```{r}
#table(balanced_data$protocol_type)
#table(data$protocol_type)
```

         b'icmp'  b'tcp'  b'udp' 
  1923    6172    11385   2520 

### Replace empty protocol_type with b'unknown':
```{r}
...
```{r cross_protocol_viz, fig.width=10, fig.height=6, warning=FALSE, message=FALSE}
...
if (length(names(protocol_data)) > 0) {
  ...
  for (protocol in names(protocol_data)) {
    if (nrow(protocol_data[[protocol]]) > 0 && "service" %in% colnames(protocol_data[[protocol]])) {
      # Get top services for this protocol
      service_counts <- table(protocol_data[[protocol]]$service)
      top_services <- names(sort(service_counts, decreasing = TRUE)[1:min(5, length(service_counts))])
  ...
  for (protocol in names(protocol_data)) {
    if (nrow(protocol_data[[protocol]]) > 0 && "flag" %in% colnames(protocol_data[[protocol]])) {
      ...
      for (flg in names(flag_counts)) {
        flag_by_protocol <- rbind(
          flag_by_protocol,
          data.frame(
            Protocol = protocol,
            Flag = flg,
            Count = flag_counts[flg],
            Percentage = 100 * flag_counts[flg] / sum(flag_counts),
            stringsAsFactors = FALSE
          )
        )
      }
