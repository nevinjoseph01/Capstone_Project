...
```
70% was recommended because our data turned out to be not even that big! wtf is then huh? mf 🤨
...
```{r}
# 4.1 Using a simple decision tree to identify most important features
using("rpart")
using("rpart.plot")

# Train a simple decision tree
tree_model <- rpart(class ~ ., data = train_imputed, method = "class", 
                   control = rpart.control(maxdepth = 5, cp = 0.01))

# Plot the tree to see which features it uses first (potential leakage)
...
# 4.2 Create datasets without potentially leaky features
# Based on correlation and importance analysis, identify suspicious features
potentially_leaky_features <- c("count", "srv_count")  # Update based on your analysis
cat("\nPotentially leaky features identified:", paste(potentially_leaky_features, collapse=", "), "\n")

# Create versions without these features
...
pred_full <- predict(model_full, data = test_imputed)
