...
Given the large magnitude of our dataset, we'll split it by protocol_type (TCP, UDP, ICMP) and create interactive visualizations for each subset to gain deeper insights into protocol-specific patterns.

```{r protocol_split}
# Split data by protocol type
protocols <- unique(data$protocol_type)
cat("Available protocols:", protocols, "\n")
...
for (protocol in protocols) {
  protocol_data[[protocol]] <- data[data$protocol_type == protocol]
...
for (protocol in protocols) {
  ...
  print(as.data.frame(table(protocol_data[[protocol]]$class)))
}
```
...
```{r tcp_visualizations, warning=FALSE, message=FALSE}
if ("tcp" %in% protocols) {
  tcp_data <- protocol_data[['tcp']]
  
  # 1. Attack Distribution in TCP
  tcp_attack_dist <- plot_ly(
    x = ~names(table(tcp_data$class)),
    y = ~as.numeric(table(tcp_data$class)),
    type = 'bar',
    marker = list(color = 'steelblue')
  ) %>%
  layout(
    title = 'Attack Type Distribution in TCP Protocol',
    xaxis = list(title = 'Attack Type', tickangle = 45),
    yaxis = list(title = 'Count', type = 'log'),
    hovermode = 'closest'
  )
  
  # 2. Duration vs Bytes Scatterplot
  tcp_bytes_plot <- plot_ly(
    data = tcp_data[sample(1:nrow(tcp_data), min(10000, nrow(tcp_data)))],
    x = ~src_bytes, 
    y = ~dst_bytes,
    color = ~class,
    type = 'scatter',
  ...
  layout(
    title = 'Source vs Destination Bytes by Attack Type (TCP)',
    xaxis = list(title = 'Source Bytes', type = 'log'),
    yaxis = list(title = 'Destination Bytes', type = 'log'),
    legend = list(x = 0.1, y = 0.9)
  )
  
  # 3. Service Distribution
  tcp_service_counts <- as.data.frame(table(tcp_data$service))
  tcp_service_counts <- tcp_service_counts[order(tcp_service_counts$Freq, decreasing = TRUE),]
  tcp_service_counts <- tcp_service_counts[1:min(15, nrow(tcp_service_counts)),]
  
  tcp_service_plot <- plot_ly(
    data = tcp_service_counts,
    x = ~Var1,
    y = ~Freq,
    type = 'bar',
  ...
  layout(
    title = 'Top 15 Services in TCP Protocol',
  ...
  tcp_error_plot <- plot_ly(
    data = tcp_data[sample(1:nrow(tcp_data), min(5000, nrow(tcp_data)))],
    x = ~rerror_rate,
    y = ~serror_rate,
    color = ~class,
    type = 'scatter',
  ...
  layout(
    title = 'Error Rate Analysis by Attack Type (TCP)',
    xaxis = list(title = 'Rerror Rate'),
    yaxis = list(title = 'Serror Rate'),
    legend = list(x = 0.1, y = 0.9)
  )
...
```{r udp_visualizations, warning=FALSE, message=FALSE}
if ("udp" %in% protocols) {
  udp_data <- protocol_data[['udp']]
  
  # 1. Attack Distribution in UDP
  udp_attack_dist <- plot_ly(
    x = ~names(table(udp_data$class)),
    y = ~as.numeric(table(udp_data$class)),
    type = 'bar',
    marker = list(color = 'forestgreen')
  ) %>%
  layout(
    title = 'Attack Type Distribution in UDP Protocol',
    xaxis = list(title = 'Attack Type', tickangle = 45),
    yaxis = list(title = 'Count', type = 'log'),
    hovermode = 'closest'
  )
  
  # 2. Duration vs Count Analysis
  udp_duration_plot <- plot_ly(
    data = udp_data,
    x = ~duration,
    y = ~count,
    color = ~class,
  ...
  layout(
    title = 'Duration vs Connection Count by Attack Type (UDP)',
    xaxis = list(title = 'Duration', type = 'log'),
    yaxis = list(title = 'Connection Count', type = 'log'),
    legend = list(x = 0.1, y = 0.9)
  )
  
  # 3. Service Distribution
  udp_service_counts <- as.data.frame(table(udp_data$service))
  udp_service_counts <- udp_service_counts[order(udp_service_counts$Freq, decreasing = TRUE),]
  udp_service_counts <- udp_service_counts[1:min(15, nrow(udp_service_counts)),]
  
  udp_service_plot <- plot_ly(
    data = udp_service_counts,
    x = ~Var1,
    y = ~Freq,
    type = 'bar',
  ...
  layout(
    title = 'Top Services in UDP Protocol',
    xaxis = list(title = 'Service', tickangle = 45),
    yaxis = list(title = 'Count', type = 'log'),
    margin = list(b = 120)
  )
  ...
  udp_bytes_boxplot <- plot_ly() %>%
    ...
    layout(
      title = 'Log Source & Destination Bytes by Attack Type (UDP)',
      xaxis = list(title = 'Attack Type', tickangle = 45),
      yaxis = list(title = 'Log10(Bytes + 1)'),
      boxmode = 'group',
      margin = list(b = 120)
    )
...
```{r icmp_visualizations, warning=FALSE, message=FALSE}
if ("icmp" %in% protocols) {
  icmp_data <- protocol_data[['icmp']]
  
  # 1. Attack Distribution in ICMP
  icmp_attack_dist <- plot_ly(
    x = ~names(table(icmp_data$class)),
    y = ~as.numeric(table(icmp_data$class)),
    type = 'bar',
    marker = list(color = 'coral')
  ) %>%
  layout(
    title = 'Attack Type Distribution in ICMP Protocol',
    xaxis = list(title = 'Attack Type', tickangle = 45),
    yaxis = list(title = 'Count', type = 'log'),
    hovermode = 'closest'
  )
  ...
  layout(
    title = 'Duration Distribution by Attack Type (ICMP)',
  ...
  # 3. Flag Distribution
  icmp_flag_counts <- as.data.frame(table(icmp_data$flag))
  
  icmp_flag_plot <- plot_ly(
    data = icmp_flag_counts,
    labels = ~Var1,
    values = ~Freq,
    type = 'pie',
    hole = 0.4,
    marker = list(colors = RColorBrewer::brewer.pal(min(8, nrow(icmp_flag_counts)), 'Set2'))
  ) %>%
  layout(
    title = 'Flag Distribution in ICMP Protocol',
    showlegend = TRUE,
    legend = list(x = 0.1, y = 0.9)
  )
  ...
  if (sum(numeric_cols) > 2) {  # Need at least 2 numeric columns
    ...
    icmp_cor <- cor(icmp_data[, numeric_cols], use = 'pairwise.complete.obs')
    # Get top correlating features (absolute value)
    abs_cor <- abs(icmp_cor)
    diag(abs_cor) <- 0  # Remove self-correlations
    # Get top N features
    n_top <- min(10, sum(numeric_cols))
    top_indices <- which(abs_cor > sort(abs_cor, decreasing = TRUE)[n_top^2], arr.ind = TRUE)
    top_features <- unique(c(row.names(icmp_cor)[top_indices[,1]], 
                            colnames(icmp_cor)[top_indices[,2]]))
    
    # Filter correlation matrix to top features
    icmp_cor_filtered <- icmp_cor[top_features, top_features]
    
    # Convert to long format for plotly
    cor_data <- expand.grid(x = colnames(icmp_cor_filtered), y = rownames(icmp_cor_filtered))
    cor_data$correlation <- as.vector(icmp_cor_filtered)
    
    icmp_heatmap <- plot_ly(
      data = cor_data,
      x = ~x,
      y = ~y,
      z = ~correlation,
    ...
    layout(
      title = 'Feature Correlation Heatmap (ICMP)',
...
```{r cross_protocol_viz, warning=FALSE, message=FALSE}
# 1. Attack type distribution across protocols
attack_by_protocol <- data.frame(Protocol=character(), AttackType=character(), Count=numeric(), stringsAsFactors=FALSE)

for (protocol in protocols) {
  proto_data <- protocol_data[[protocol]]
  class_counts <- table(proto_data$class)
...
protocol_attack_dist <- plot_ly(
  data = attack_by_protocol,
  x = ~Protocol,
  y = ~Count,
  color = ~AttackType,
  type = 'bar'
...
layout(
  title = 'Attack Type Distribution Across Protocols',
  xaxis = list(title = 'Protocol'),
  yaxis = list(title = 'Count', type = 'log'),
  barmode = 'stack',
...
sample_size_per_protocol <- 5000

protocol_samples <- data.frame()
for (protocol in protocols) {
  proto_data <- protocol_data[[protocol]]
  # Sample from each protocol
  n_to_sample <- min(sample_size_per_protocol, nrow(proto_data))
  sampled_rows <- proto_data[sample(1:nrow(proto_data), n_to_sample),]
  sampled_rows$Protocol <- protocol  # Add protocol identifier
  protocol_samples <- rbind(protocol_samples, sampled_rows)
}

# Create 3D scatter plot comparing key features
feature_3d <- plot_ly(
  data = protocol_samples,
  x = ~src_bytes,
  y = ~dst_bytes,
  z = ~duration,
  color = ~Protocol,
...
layout(
  title = '3D Comparison of Key Features Across Protocols',
  scene = list(
    xaxis = list(title = 'Source Bytes', type = 'log'),
    yaxis = list(title = 'Destination Bytes', type = 'log'),
    zaxis = list(title = 'Duration', type = 'log')
  )
)

# 3. Connection metrics comparison
metrics_boxplot <- plot_ly() %>%
  add_boxplot(
    data = protocol_samples[protocol_samples$Protocol == protocols[1],],
    y = ~same_srv_rate,
    name = protocols[1],
    boxpoints = 'outliers',
    jitter = 0.3,
...
if (length(protocols) > 1) {
  for (i in 2:length(protocols)) {
    metrics_boxplot <- metrics_boxplot %>%
      add_boxplot(
        data = protocol_samples[protocol_samples$Protocol == protocols[i],],
        y = ~same_srv_rate,
        name = protocols[i],
        boxpoints = 'outliers',
        jitter = 0.3,
        marker = list(color = RColorBrewer::brewer.pal(length(protocols), 'Set1')[i]),
        line = list(color = RColorBrewer::brewer.pal(length(protocols), 'Set1')[i]),
        x = 'Same Service Rate'
      )
  }
}
...
metrics_boxplot <- metrics_boxplot %>%
  layout(
    title = 'Comparison of Same Service Rate Across Protocols',
    yaxis = list(title = 'Rate Value'),
    boxmode = 'group'
  )

# Display cross-protocol visualizations
protocol_attack_dist
feature_3d
metrics_boxplot
```
