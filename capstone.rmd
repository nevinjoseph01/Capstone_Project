...
We can see that our data has 41 features and 1 target variable. The target variable is the last column, and it is a categorical variable with 25 classes. The data consists of over half a million observations (records). The first column is the duration of the connection in seconds, and the rest are various features related to the connection. _Note_ that class "Normal" refers to nonanomalous connections. There are no binary features. There are a few categorical features, which we need to factor at some point.
...
```
Since our given data set was unlabeled, we named all the features with accordance to the reference paper.


### Checking for class distribution

```{r}
freq_table <- as.data.frame(table(data$class))

# Sort by frequency (i.e., the 'Freq' column)
sorted_freq_table <- freq_table[order(-freq_table$Freq), ]

# View result
print(sorted_freq_table)
```

* Having analysed the target distribution, we see a significant difference in the number of samples for each class. This most likely will be handled to avoid data leakage. The difference between the most frequent max class and min class is about 154,000 samples. 
* Another important thing we spotted is that there is a nameless class. We found no usefulness for it and therefore we will remove it.
...
```
Since we observed classes with frequency less than 100, we decided to remove them as they caused unavoidable data leakage. 

```{r}
cat("are there any 'not available' data?", any(is.na(data))) # 
cat("In which columns those missing data exist:", colSums(is.na(data))) #
...
```
The sum of duplicates is 128935, meaning our new data will have 399668 samples. 
...
#### Remove samples with empty class
...
#View(data)
```
Median imputation values is done now. We didn't use mean because median would be less sensitive to outliers.

#### Removing outliers (EDA, but not practiced)
...
# 0.4350536
```
We kept the option of having a dataset without outliers for future use, but we decided to use the original dataset with outliers as we found usefulness and value in the outliers for detecting certain types of attacks. 
...
### Checking for overlaping values (EDA, but not proccessed):
...
#### Potential categorizing the whole dataset according to flag, service and protocol_type:
```{r}
...
length of flag: 12
length of service: 67
length of protocol_type: 4
...
### Setting a balanced target for each class

```{r}
target_samples <- 2000  # 
```
We'll aim for 2000 samples per class

The following is to create a balanced dataset using both undersampling and oversmampling using SMOTE
```{r}
...
```
Note, the oversampling technique is not unitlized by duplicating minorities, but by creating synthetic samples using SMOTE.
